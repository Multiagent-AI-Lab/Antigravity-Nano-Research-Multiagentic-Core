...
1. **Paper académico en `paper/`** (`paper.md`, `paper.bib`, 2 figuras) — formato JOSS/JOSE, con ORCID del profesor, fecha 2026, título "*Antigravity Nano Research Multiagentic Core: An Open Educational Framework for Multi-Agent AI in Computational Nanotechnology*". El paper **describe la arquitectura multi-agente y RAG como si ya existieran** — es decir, describe la promesa del repo, no su estado real (0 agentes, RAG mock) previo a este trabajo. Si este paper ya fue sometido o publicado en algún lado, hay una discrepancia entre lo publicado académicamente y el código real que existía hasta ahora — **verificar el estado de sometimiento antes de decidir si el paper se actualiza (para que pase a describir con precisión la arquitectura real que este plan construye) o se retira**. Esto no es opcional: un paper académico con afirmaciones que no correspondían al código es una cuestión de integridad académica, no solo de completitud del repo.
